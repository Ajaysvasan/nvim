...
| `SPC s s` | Sessions | `:lua require('persistence').load()` |
...
> ⚠️ **Known issue:** the *Sessions* button calls `persistence.nvim`, which is
> **not in the plugin list**. Pressing it errors with "module 'persistence' not
> found". Either add `folke/persistence.nvim` to `plugins.lua` or remove that
> button.
