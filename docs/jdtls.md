...
> ⚠️ `<leader>jn` here (test nearest method) collides with `<leader>jn` in
> `java-creator.lua` (new Java file GUI). Currently harmless because java-creator
> is **not loaded** — see [java-creator.md](java-creator.md).
