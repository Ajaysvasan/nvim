...
Loaded from the `nvim-jdtls` spec (`ft = "java"`), so it costs nothing until you
open a Java file — it is ~1100 lines and only meaningful inside a Java project.
...
> **Was `<leader>jn`, which [jdtls.lua](jdtls.md) also maps** — to "test nearest
> method", buffer-locally, in exactly the Java buffers where you would want the
> creator. Buffer-local mappings beat global ones, so jdtls silently won and this
> never fired. jdtls keeps `<leader>jn`; the creator takes the shifted variant.
...
`plugins.lua`, in the `nvim-jdtls` spec:
...
Registration moved into an `M.setup()` so the module matches every other one in
the config and can go through `setup_module()`, which reports *which* file failed
instead of "attempt to index a boolean value". Requiring it now has no side
effects.

Because it loads with Java, `:JavaNew` becomes available once any Java file is
open. To create the very first file in an empty project, use neo-tree's `a`.
