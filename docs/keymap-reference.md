...
| `<C-1>` … `<C-5>` | n | Jump to Harpoon slot 1–5 | [harpoon](harpoon.md) |
...
| `gc` | ⚠️ **Contested** — `:LazyGitConfig` *or* Telescope git commits | lazygit / telescope |
...
| `jm` | v | Extract method |
| `jt` | n | Run test class |
| `jn` | n | Run nearest test method ⚠️ *(collides with `:JavaNew` if java-creator is enabled)* |
...
| `tt` | Transparency *(only if `transparency.lua` is loaded)* | [inactive](inactive-modules.md) |
...
| Key | Claimed by | Resolution |
|---|---|---|
| `<leader>gc` | LazyGit config **and** Telescope git commits | Both global; last loaded wins. Rename one. |
| `<leader>hd` | Gitsigns "diff this" **and** Harpoon "remove file" | Gitsigns' is buffer-local, so it wins inside git-tracked files |
| `<leader>jn` | jdtls "test nearest" **and** java-creator `:JavaNew` | Harmless today — java-creator isn't loaded |
| `<C-k>` | Window-up **and** LSP signature help | LSP version is buffer-local; use `<C-w>k` in LSP buffers |
| `K` | Move selection up (visual) **and** LSP hover (normal) | Different modes — no actual conflict |
| `<C-1>`–`<C-5>` | Harpoon, but the lazy-load trigger declares `<A-1>`–`<A-5>` | Load harpoon via `<leader>a` first; many terminals don't send these keys at all |
